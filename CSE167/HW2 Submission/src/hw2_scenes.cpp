...
// 4 triangles, 4th triangle behind camera. Should output all but pink triangle
TriangleMesh testmesh0 {
    // vertices
    {{-0.483444, 0.826069, -2.307457},
     {1.152029, -1.111880, -1.928058},
     {1.485437, 0.880605, -2.239198},
    ...
    {{0.35, 0.75, 0.35}, {0.35, 0.75, 0.35}, {0.35, 0.35, 0.75}, {0.75, 0.35, 0.75}},
    // vertex_colors
    {{0.75, 0.35, 0.35}, {0.35, 0.75, 0.35}, {0.35, 0.35, 0.75}, {0.75, 0.35, 0.75}, {0.75, 0.35, 0.75}}
};

// two triangles. First behind clipping, second in front. Should output only blue triangle
TriangleMesh testmesh1 {
    // vertices
    {{-1.7, 1.0, -5.0}, {1.0, 1.0, 5.0}, {-0.5, -1.0, -5.0}, {-2.5, -1.0, -5.0}},
    // faces
    {{0, 1, 2}, {0, 2, 3}},
...
// two triangles. Both behind clipping. Should output only background.
TriangleMesh testmesh2 {
    // vertices
    {{-1.7, 1.0, 5.0}, {1.0, 1.0, -5.0}, {-0.5, -1.0, -5.0}, {-2.5, -1.0, -5.0}},
    // faces
    {{0, 1, 2}, {0, 2, 3}},
...
std::vector<TriangleMesh> meshes = {mesh0, mesh1, mesh2, mesh3, testmesh0, testmesh1, testmesh2};
...
Matrix4x4 parse_transformation(const json &node) {
    ...
    for (auto it = transform_it->begin(); it != transform_it->end(); it++) {
        ...
        if (auto scale_it = it->find("scale"); scale_it != it->end()) {
            ...
            m(0,0) = scale.x;    /* [sx,_,_,_] */
            m(1,1) = scale.y;    /* [_,sy,_,_] */
            m(2,2) = scale.z;    /* [_,_,sz,_] */
            F = m * F;           /* [_,_,_, 1] */
            ...
            // first column
            m(0,0) = axis.x * axis.x + (1 - axis.x * axis.x)   * std::cos(radians);
            m(1,0) = axis.x * axis.y * (1 - std::cos(radians)) + axis.z * std::sin(radians);
            m(2,0) = axis.x * axis.z * (1 - std::cos(radians)) - axis.y * std::sin(radians);
            m(3,0) = 0;

            // second column
            m(0,1) = axis.y * axis.x * (1 - std::cos(radians)) - axis.z * std::sin(radians);
            m(1,1) = axis.y * axis.y + (1 - axis.y * axis.y)   * std::cos(radians);
            m(2,1) = axis.y * axis.z * (1 - std::cos(radians)) + axis.x * std::sin(radians);
            m(3,1) = 0;
 
            // third column
            m(0,2) = axis.z * axis.x * (1 - std::cos(radians)) + axis.y * std::sin(radians);
            m(1,2) = axis.z * axis.y * (1 - std::cos(radians)) - axis.x * std::sin(radians);
            m(2,2) = axis.z * axis.z + (1 - axis.z * axis.z)   * std::cos(radians);
            m(3,2) = 0;
            F = m * F;
            ...
            m(0,3) = translate.x; /* [1,_,_,tx] */
            m(1,3) = translate.y; /* [_,1,_,ty] */
            m(2,3) = translate.z; /* [_,_,1,tz] */
            F = m * F;            /* [_,_,_, 1] */
            ...
            Vector3 d = normalize(target - position); // 1) get direction starting from position to target
            Vector3 r = normalize(cross(d, up));      // 2) compute right vector = cross product of d and u (up)
            Vector3 up_p = cross(r, d);               // 3) recompute up vector to be orthogonal to camera direction vector

            /* std::cout << "d: " << d << std::endl;
            std::cout << "r: " << r << std::endl;
            std::cout << "u': " << up_p << std::endl; */
